...
**主要来源（优先使用）**：抓取 arXiv 官方每日列表页面，获取当天最新论文 ID：

1. `https://arxiv.org/list/cs.SD/new` — Sound 分类（仅今日新提交）
2. `https://arxiv.org/list/eess.AS/new` — Audio and Speech Processing 分类（仅今日新提交）

从页面中提取**当天所有 arXiv ID**，合并去重。`/new` 页面按当日批次展示，通常包含 3 个分区：
- `New submissions`
- `Cross submissions`
- `Replacement submissions`

> 实操建议：如果当前环境没有 `web_fetch/read_arxiv_paper/search_arxiv` 这类工具，直接用 `python + requests + beautifulsoup4` 抓取并解析 HTML 即可。
...
## 第三步：交付（本地 Markdown / 腾讯文档）

### 3A. 本地输出到桌面（Markdown 表格）

当用户要求“输出到桌面、保存为 md、内容为表格”时，按以下格式生成并写入：

- **输出路径（Windows）**：`%USERPROFILE%\\Desktop\\YYYY-MM-DD_语音_今日新论文.md`
- **表格列（推荐）**：
  - 状态（New/Cross/Replacement）
  - arXiv ID
  - 标题
  - Submitted on（从 `https://arxiv.org/abs/<ID>` 的 dateline 提取）
  - 主分类（primary subject）
  - 来源（`cs.SD` / `eess.AS`）
  - Abs / PDF
  - 代码/Demo（abs 页中抓取 `github.com` / `*.github.io` / `huggingface.co` 等外链；无则留空）

> 注意：在上海时区的早晨，arXiv 的 `/new` 页面常仍显示前一自然日批次（例如 2026-03-24 早晨看到 “Monday, 23 March 2026”），以页面显示的批次为准，并在 md 头部注明“列表批次”和“生成时间”。
